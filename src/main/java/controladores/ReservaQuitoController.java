...
@WebServlet("/ReservaQuitoController")
public class ReservaQuitoController extends HttpServlet{
	...
	private void guardarReserva(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	    try {
	        ...
	        String idSucursal = "Quito"; // Aquí puedes ajustar la lógica según la sucursal
	        ...
	        ReservaQuitoDAO reservaDAO = new ReservaQuitoDAO();
	        ...
	        if (reserva.getIdReserva() > 0) { // Verificar si la reserva se creó correctamente
	            ...
	            if (servicioInsertado && mascotaInsertada) {
	                ...
	                req.getRequestDispatcher("QuitoViewController?ruta=solicitarReservaQuito").forward(req, resp);
	            } else {
